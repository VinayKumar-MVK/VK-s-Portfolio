...
const PROJECT_DATA = PROJECTS.find((p) => p.slug === "giet-smart")!;

const FEATURES = [
  {
    title: "Firebase Auth Security",
    description: "Multi-provider authentication via institutional email address, OAuth Google Sign-In, and encrypted session handling.",
    icon: ShieldCheck,
  },
  {
    title: "Attendance & Grade Tracker",
    description: "Live academic progress metrics with threshold warnings, subject attendance percentages, and internal marks summary.",
    icon: GraduationCap,
  },
  {
    title: "Smart Notification Alerts",
    description: "Automated alert triggers reminding students of upcoming assignments, exam schedules, and fee deadlines.",
    icon: BellRing,
  },
  {
    title: "Low-Bandwidth Optimization",
    description: "Local SQLite caching layer enabling seamless offline access to timetables and notes during weak campus coverage.",
    icon: WifiOff,
  },
];

export default function GietSmart() {
  const otherProjects = PROJECTS.filter((p) => p.slug !== "giet-smart").slice(0, 3);
    ...
    <PageWrapper>
      ...
      <div className="container mx-auto px-4 sm:px-6 max-w-6xl">
        ...
        >
          ...
          <ChevronRight size={14} />
          <span className="text-white font-medium">Giet Smart</span>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-8 mb-8 sm:mb-12 md:mb-14">
          ...
          <div className="lg:col-span-2 space-y-5 sm:space-y-8">
            ...
            >
              <TiltCard
                maxTilt={8}
                glareOpacity={0.18}
                className="p-4 sm:p-6 md:p-8 border-white/10 touch-manipulation hover:border-red-500/40 transition-colors bg-white/[0.02] backdrop-blur-sm"
              >
                <h2 className="text-lg sm:text-xl font-black mb-3 sm:mb-5 flex items-center gap-2 text-white">
                  <Layers size={18} className="text-red-400" /> About Giet Smart
                </h2>
              ...
              {FEATURES.map((feat, idx) => {
                ...
                return (
                  <TiltCard
                    ...
                    maxTilt={10}
                    glareOpacity={0.2}
                    className="p-4 sm:p-6 border-white/10 touch-manipulation hover:border-red-500/40 transition-colors bg-white/[0.02]"
                  >
                    <div className="w-10 h-10 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center justify-center mb-4 text-red-400">
            ...
            {PROJECT_DATA.gallery.length > 0 && (
              ...
              >
                <TiltCard
                  maxTilt={6}
                  glareOpacity={0.15}
                  className="p-4 sm:p-6 md:p-8 border-white/10 touch-manipulation hover:border-red-500/30 transition-colors bg-white/[0.02]"
                >
                  <h2 className="text-lg sm:text-xl font-black mb-3 sm:mb-5 text-white">Application Screenshots</h2>
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            >
              <div
                className="p-4 sm:p-6 border-white/10 touch-manipulation hover:border-red-500/30 transition-colors bg-white/[0.02]"
              >
                <h3 className="text-sm sm:text-base font-bold text-white mb-3 sm:mb-4">Project Links</h3>
                <div className="space-y-3">
                  <a
                    href="https://github.com/VinayKumar-MVK/Giet_Smart"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-red-500/10 text-red-400 border border-red-500/30 hover:bg-red-500/20 hover:border-red-500/60 transition-all font-bold text-sm"
                  >
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {PROJECT_DATA.tech.map((t) => (
                    <span
                      key={t}
                      className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 active:bg-red-500/20 transition-colors"
                    >
                      {t}
                    </span>
                  ))}
