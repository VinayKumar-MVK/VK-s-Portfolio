...
const PROJECT_DATA = PROJECTS.find((p) => p.slug === "deep-dive-music-player")!;

const DESIGN_PILLARS = [
  {
    title: "Dark Glassmorphism Aesthetic",
    description: "Curated translucent backdrop blurs, neon accent highlights, and subtle drop shadows for high depth UI.",
    icon: Palette,
  },
  {
    title: "Waveform & Equalizer Visualizer",
    description: "Interactive dynamic spectrum visualizer tailored for real-time acoustic output representations.",
    icon: Activity,
  },
  {
    title: "Micro-Animations & Gestures",
    description: "Figma smart-animate components, swipeable tracks, expandable mini-player, and tactile playback controls.",
    icon: Sparkles,
  },
  {
    title: "Design System & Tokens",
    description: "Unified typography hierarchy, component variants, light/dark color tokens, and atomic design library.",
    icon: Figma,
  },
];

export default function DeepDiveMusicPlayer() {
  const otherProjects = PROJECTS.filter((p) => p.slug !== "deep-dive-music-player").slice(0, 3);
    ...
    <PageWrapper>
      ...
      <div className="container mx-auto px-4 sm:px-6 max-w-6xl">
        ...
        >
          ...
          <ChevronRight size={14} />
          <span className="text-white font-medium">Deep Dive Music Player UI</span>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-8 mb-8 sm:mb-12 md:mb-14">
          ...
          <div className="lg:col-span-2 space-y-5 sm:space-y-8">
            ...
            >
              <TiltCard
                maxTilt={8}
                glareOpacity={0.18}
                className="p-4 sm:p-6 md:p-8 border-white/10 touch-manipulation hover:border-emerald-500/40 transition-colors bg-white/[0.02] backdrop-blur-sm"
              >
                <h2 className="text-lg sm:text-xl font-black mb-3 sm:mb-5 flex items-center gap-2 text-white">
                  <Music size={18} className="text-emerald-400" /> UI Concept & Design Narrative
                </h2>
              ...
              {DESIGN_PILLARS.map((pillar, idx) => {
                ...
                return (
                  <TiltCard
                    ...
                    maxTilt={10}
                    glareOpacity={0.2}
                    className="p-4 sm:p-6 border-white/10 touch-manipulation hover:border-emerald-500/40 transition-colors bg-white/[0.02]"
                  >
                    ...
                    <h3 className="text-white font-bold text-sm sm:text-base mb-1.5 sm:mb-2">{pillar.title}</h3>
            ...
            >
              ...
              >
                <h2 className="text-lg sm:text-xl font-black mb-3 sm:mb-5 flex items-center gap-2 text-white">
                  <CheckCircle2 size={18} className="text-secondary" /> Prototype Highlights
                </h2>
            ...
            {PROJECT_DATA.gallery.length > 0 && (
              ...
              >
                <TiltCard
                  maxTilt={6}
                  glareOpacity={0.15}
                  className="p-4 sm:p-6 md:p-8 border-white/10 touch-manipulation hover:border-emerald-500/30 transition-colors bg-white/[0.02]"
                >
                  <h2 className="text-lg sm:text-xl font-black mb-3 sm:mb-5 text-white">Design Screenshots</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    {PROJECT_DATA.gallery.map((img, i) => (
                      <div key={i} className="rounded-xl overflow-hidden border border-white/10 aspect-video">
                        <img
                          src={img}
                          alt={`Deep Dive Music Player screenshot ${i + 1}`}
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            >
              <div
                className="p-4 sm:p-6 border-white/10 touch-manipulation hover:border-emerald-500/30 transition-colors bg-white/[0.02]"
              >
                <h3 className="text-sm sm:text-base font-bold text-white mb-3 sm:mb-4">Figma Design Links</h3>
                <div className="space-y-3">
                  <a
                    href="https://www.figma.com/design/8SG3XJSnEphy5xCwFviLzx/Deepdive?node-id=0-1&t=ciuCAU5cKavTrLlQ-1"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/20 hover:border-emerald-500/60 transition-all font-bold text-sm"
                  >
                    <ExternalLink size={15} /> Figma Prototype Link
                  </a>
                  <Link href="/projects">
            ...
            >
              ...
              >
                <h3 className="text-sm sm:text-base font-bold text-white mb-3 sm:mb-4">Design Tools</h3>
                <div className="flex flex-wrap gap-2">
                  {PROJECT_DATA.tech.map((t) => (
                    <span
                      key={t}
                      className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 active:bg-emerald-500/20 transition-colors"
                    >
                      {t}
                    </span>
                  ))}
            ...
            >
              ...
              >
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center border-b border-white/5 pb-3">
                    <span className="text-muted-foreground">Platform Target</span>
                    <span className="text-white font-medium">iOS & Desktop GUI</span>
                  </div>
